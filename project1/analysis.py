...
class Analysis:
    ...
    def mean(self, headers, rows=[]):
        ...
        return np.sum(select, axis=0)*select.shape[0]**(-1)
    ...
    def var(self, headers, rows=[]):
        ...
        '''
        select= self.data.select_data(headers,rows)
        meanCols= self.mean(headers, rows)
        sum= np.sum((select-meanCols)**2, axis=0)
        return sum* (select.shape[0]-1)**(-1)
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 13), title=''):
        ...
        fig, axes= plt.subplots(x,x,figsize= fig_sz, sharex='col', sharey='row')
        fig.suptitle(title)

        for i in range(x):
            for j in range(x):
                axes[i][j].scatter(data[:, j], data[:,i], marker ='o')
                if i== x-1:
