...
class LinearRegression(analysis.Analysis):
    ...
    def linear_regression(self, ind_vars, dep_var):
        ...
        yhat = self.predict()
        self.residuals = self.compute_residuals(yhat)
        self.R2 = self.r_squared(yhat)
    ...
    def compute_mse(self):
        ...
        '''
        yhat = self.predict()
        residSum= np.sum(self.compute_residuals(yhat)**2)
        mse = float((residSum)/(self.y.shape[0]))
        return mse
    ...
    def scatter(self, ind_var, dep_var, title):
        ...
        if self.p == 1:
            ...
            x_poly_line = self.make_polynomial_matrix(x_line, self.p)
            y_line = x_poly_line @ self.slope + self.intercept

        plt.scatter(x,y)
        plt.plot(x_line, np.squeeze(y_line), color = 'brown')
        plt.title(f"{title}, R^2 = {self.R2:.3f}")
        plt.xlabel(ind_var) 
        plt.ylabel(dep_var)
    ...
    def pair_plot(self, data_vars, fig_sz=(12, 12), hists_on_diag=True):
        ...
        for i in range (x):
            for j in range (x):
                ...
                axes[i, j].plot(line_x, line_y, '', color='brown')
                axes[i, j].set_title(f'R^2 = {self.R2:0.3f}')
    ...
    def poly_regression(self, ind_var, dep_var, p):
        ...
        self.y = y
        N = A.shape[0]
        Ap = self.make_polynomial_matrix(A, p)
        ones = np.ones((N,1))
        Ahat= np.hstack((ones, Ap))
