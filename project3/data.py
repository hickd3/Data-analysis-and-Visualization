...
class Data:
    ...
    def read(self, filepath):
        ...
        vartypes = file.readline().split(',')
        try:
            for vartype in vartypes:
                title = vartype.strip()
                if title not in ['categorical', 'numeric', 'string', 'date']:
                    print(x)
        except:
            print("Error parsing variable types")
        ...
        for x in vartypes:
            ...
            if type == 'numeric':
                ...
                title = raw_headers[counter].replace('\n','')
                headers.append(title.strip())
            elif type == 'categorical':
                valid_columns.append(counter)
                title = raw_headers[counter].replace('\n','')
                headers.append(title.strip())
                self.cats2levels[title] = [] 
            counter += 1
        ...
        for line in file:
            ...
            for index, data in enumerate(contents):
                data = data.replace('\n', '').strip()
                if index in valid_columns:
                    name = raw_headers[index].replace('\n', '').strip()
                    if data == '':
                        if name in self.cats2levels:
                            if 'Missing' not in self.cats2levels[name]:
                                self.cats2levels[name].append('Missing')
                            newline.append(self.cats2levels[name].index('Missing'))
                        else:
                            newline.append(np.nan)
                    else:
                        if name in self.cats2levels:
                            if data not in self.cats2levels[name]:
                                self.cats2levels[name].append(data)
                            newline.append(self.cats2levels[name].index(data))
                        else:
    ...
    def __str__(self):
        ...
        rowSize = min(5, len(self.data))

        for i in range(rowSize):
            rowStr= ','.join(map(str, self.data[i]))
            string += rowStr + '\n'
        return string.strip()
    ...
    def get_header_indices(self, headers):
        ...
        indicies = []
        for i in headers:
            indicies.append(self.headers.index(i))
        return indicies
    ...
    def select_data(self, headers, rows=[]):
        ...
        if len(rows) != 0:
            result.append(self.data[np.ix_(rows, indices)])
        else:
            x= np.arange(self.data.shape[0])
            result.append(self.data[np.ix_(x, indices)])
